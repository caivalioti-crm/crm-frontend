...
export function NewVisitDialog({ isOpen, onClose, customers, onSave }: NewVisitDialogProps) {
  ...
  const areas = useMemo(() => [...new Set(customers.map(c => c.area).filter(Boolean))].sort() as string[], [customers]);
  ...
  const convertDateToISO = (ddmmyy: string): string => {
    const parts = ddmmyy.split('/');
    if (parts.length !== 3) return '';
    const [day, month, shortYear] = parts;
    const year = Number(shortYear) < 50 ? `20${shortYear}` : `19${shortYear}`;
    return `${year}-${month}-${day}`;
  };

  const getRemiderDate = (type: string): string => {
    const base = new Date();
  ...
  const handleAddTask = () => {
    ...
    if (newTaskReminderType === 'custom') {
      task.reminderDate = convertDateToISO(newTaskCustomDate);
    } else if (newTaskReminderType) {
      task.reminderDate = getRemiderDate(newTaskReminderType);
    }
    setTasks([...tasks, task]);
  ...
  const handleSave = async () => {
    ...
    const isoDate = convertDateToISO(visitDate);
    if (!isoDate) { setError('Invalid date format — use dd/mm/yy'); return; }
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={visitDate}
                onChange={e => setVisitDate(e.target.value)}
                placeholder="30/04/26"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Time (optional, 24h)</label>
              <input type="text" value={visitTime}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {newTaskReminderType === 'custom' && (
                  <input type="text" value={newTaskCustomDate} onChange={e => setNewTaskCustomDate(e.target.value)}
                    placeholder="dd/mm/yy" className="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" />
                )}
              </div>
